...
const rootReducer = (state = intialState, action) => {
  switch (action.type) {
    case "GET_ALL_DOGS":
      action.payload.forEach(element => {
        if (!element.temperaments[0]) {
          element.temperaments[0] = "no-temperaments" //aqui elimino los arrays vacios de temperamentos
        }
      });
      return {
        ...state,
    ...
    case "GET_TEMPERAMENTS":
      const filteresTemp = action.payload.filter((temp) => temp.name !== ""); //aqui elimino las razas con strings vacios
      return {
        ...state,
